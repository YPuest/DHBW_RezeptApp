...
export default function Favorites() {
    ...
    useEffect(() => {
        ...
        if (favorites) {
            ...
            const favoriteRecipePreviews = recipes
                .filter(recipe => favoriteNames.includes(recipe.description.name))
                .map((recipe, index) => (
                    <RecipePreview
                        key={index}
    ...
    return (
        <div className="p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {favoriteRecipes.length > 0 ? favoriteRecipes : <p>Keine Favoriten gefunden</p>}
            </div>
        </div>
    );
}
